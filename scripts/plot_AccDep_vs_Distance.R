...
# comp_eq_subset = subset( complete_output_data, timestep_index == time_index & equation == "SE1" & as.numeric(station) > 1)
...
p_comp_eq  = p_comp_eq  + labs( title = "Accumulated deposition for last timestep (29h)",
                                subtitle = "Equation comparison",
                                x = "Distance from downstream station (m)" ,
                                y = "Accumulated deposition (tons)", color = "Equation")
# Matrix plot:
...
label_data_up = subset( comp_eq_subset ,
                      return_period == 2 & station %in% stations_up & equation == "SE11")

stations_down = c(8, 12, 16, 19, 36, 44, 49, 57, 67, 94, 101, 106, 137)
label_data_down = subset( comp_eq_subset ,
                        return_period == 2 & station %in% stations_down & equation == "SE11" )
